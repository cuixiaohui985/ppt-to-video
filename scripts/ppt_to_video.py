...
def build_video_with_typewriter(
    ...
    带打字机效果（正文页）的视频合成。
    封面页：直接使用原图（无效果）
    正文页：用 PIL 透明文字图 + ffmpeg crop 渐进裁剪逐字渐入
    """
    temp_dir = os.path.dirname(output_path)
...
def main():
    parser = argparse.ArgumentParser(description="PPT 转视频工具（优化版 v0.3）")
